    ...
    public Publisher<Void> onPing(UdpInbound udpInbound, UdpOutbound udpOutbound) {
        return udpInbound.receiveObject()
                ...
                .flatMap(datagramPacket -> {
                    try {
                        Ping ping = PingUtils.toPing(datagramPacket);
                        logOnPing(ping);
                        checkPing(ping);
                        ...
                        if (ping.getDirect()) {
                            ...
                            Mono<Ack> nackMono = Mono.just(AckUtils.nack(ack)).delayElement(Duration.ofMillis(ping.getNackTimeout()));
                            DatagramPacket nackDatagram = AckUtils.toDatagram(AckUtils.nack(ack), datagramPacket.sender());
